...
class ResNetVAEencoder(nn.Module): # first_ch=16 second_ch=32 third_ch=64 forth_ch=128
    def __init__(self, first_ch, second_ch, third_ch, z_ch):
        super(ResNetVAEencoder, self).__init__()
        self.third_ch = third_ch
        ...
        self.relu = nn.ReLU()
        ## 低次元空間のサイズ設定
        self.fc = nn.Linear(third_ch*10*14*10, z_ch*2)
        # self.down = nn.Linear(z_ch, 50)
        # n_classes = 3
        self.Wstar_layer = nn.Linear(z_ch, 3, bias=False)  # 'bias=False' allows the weights to be the pure representation vectors (not affected by the bias)
    ...
    def forward(self, x):
        ...
        x = self.relu(x+h) # here add
        x = x.view(x.size(0), -1)
        x = self.fc(x)  #  z = 350dim
        mu, logvar = x.chunk(2, dim=1) #  一定の次元で分割

        # z = self.down(mu) # 50dim
        ...
        # Wstar が 175dim のベクトルの3プロトタイプ (Wstar が 50dim のベクトルの3プロトタイプ)
        # Compute cosine similarities between the normalized representation vectors and the prototype vectors
        # Note that the representations are normalized to have unit length
...
class ResNetDecoder(nn.Module):
    def __init__(self, first_ch, second_ch, third_ch, z_ch):
        ...
        self.dfc = nn.Sequential(
            ##  低次元空間のサイズを決める
            nn.Linear(z_ch, third_ch*1400), # 10*14*10 = 1400
            nn.ReLU(True),
        )
        self.block1 = nn.Sequential(
            nn.Conv3d(third_ch, third_ch, kernel_size=3, stride=1, padding=1, bias=True),
        ...
        self.block2u = nn.Sequential(
            ...
            nn.Conv3d(third_ch, third_ch, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm3d(third_ch),
            # nn.LeakyReLU(0.2, inplace=True), #skip
        )
        self.block3u = nn.Sequential(
            nn.Conv3d(third_ch, third_ch, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm3d(third_ch),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2, mode='nearest'), # 20*28*20
        ...
        self.block4u = nn.Sequential(
            ...
            nn.BatchNorm3d(second_ch),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2, mode='nearest'), # or  nn.Upsample(scale_factor=2, mode="trilinear", align_corners=True),
        ...
        self.block5u = nn.Sequential(
            ...
            nn.BatchNorm3d(first_ch),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Upsample(scale_factor=2, mode='nearest'), # 80*112*80
    ...
    def forward(self, z):
        y = z.view(z.size(0), -1)
        y = self.dfc(y)
        y = y.view(y.size(0), self.third_ch, 10, 14, 10)
        y = self.block1(y) 
        h = self.block2u(y) #### -- 10*14*10
        y = self.dLeakyrelu1(y+h) # here add
        y = self.block3u(y) #### -- 20*28*20
        y = self.block4u(y) #### -- 40*56*40
        y = self.block5u(y) #### -- 80*112*80
        y = self.last_block(y)
        return y


class ResNetVAE(BaseVAE):
    def __init__(self, first_ch, second_ch, third_ch,  z_ch) -> None:
        super(ResNetVAE, self).__init__()
        self.encoder = ResNetVAEencoder(first_ch, second_ch, third_ch, z_ch)
        self.decoder = ResNetDecoder(first_ch, second_ch, third_ch, z_ch)
