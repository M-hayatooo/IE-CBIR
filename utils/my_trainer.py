...
def train_RaDOGAGA_VAE(
    ...
    train_d1_losses, train_d2_losses, val_d1_losses, val_d2_losses = [], [], [], []
    train_kl_losses, val_kl_losses = [], []
    print(f"beta_d1(mse)={beta_d1}  beta_d2={beta_d2}  beta_kl={beta_kl}")
    start_time = time.time()
    for epoch in range(epochs):
        loop_start_time = time.time()
        train_run_d1, train_run_d2, train_run_kl = 0.0, 0.0, 0.0
        train_run_loss = 0.0
        net.train()
        for inputs, labels in train_loader:
            inputs = inputs.to(device)
            optimizer.zero_grad()
            x_rec_z, x_rec_mu, mu, logvar = net(inputs) #  forward: return  x_rec_z, x_rec_mu, mu, logvar
            # loss, mse, kl = lossf.normal_loss(x_re, mu, logvar, inputs)
            d1 = lossf.mse_loss(inputs, x_rec_mu) #  入力とエンコードして得られた平均 mu から再構成した誤差をd1
            d2 = lossf.mse_loss(x_rec_mu, x_rec_z) # 平均 mu とサンプリングした z から再構成した画像それぞれの再構成誤差をd2 とし減らそうとする.
            kl = lossf.kld_loss(mu, logvar)
            loss = (beta_d1)* d1 + (beta_d2)* d2 + (beta_kl)* kl
    ...
    # __________________________________________________________________________________#
        ...
        train_d1_losses.append(train_run_d1)
        train_d2_losses.append(train_run_d2)
        train_kl_losses.append(train_run_kl)
# __________________________________________________________________________________ #

        net.eval()
        val_run_d1, val_run_d2, val_run_kl = 0.0, 0.0, 0.0
        val_run_loss = 0.0
        with torch.no_grad():
    ...
    #       torch.save(model.state_dict(), log_path + f"softintroVAE_weight_epoch{str(epoch)}.pth")
        ...
        print(f"Epoch[{epoch+1}/{epochs}] "
              f"Train[d1RMSE:{train_run_d1:.1f} d2RMSE:{train_run_d2:.1f} kl:{train_run_kl:.2f} loss:{train_run_loss:.1f}]  "
              f"Val[d1RMSE:{val_run_d1:.1f} d2RMSE:{val_run_d2:.1f} kl:{val_run_kl:.2f} loss:{val_run_loss:.1f}]  "
    ...
    write_kl_losses_onlyvae(path + "/train_losses.txt", train_d1_losses, train_kl_losses)
    write_kl_losses_onlyvae(path + "/val_losses.txt", val_d1_losses, val_kl_losses)


    if epochs != 0:
        net = net.to('cpu') # =============================================================================
        torch.save(net.state_dict(), path + "RaDOvae_last_weight.pth")
        print("saved RaDOGAGAVAE param and ", end="")

    print("finished RaDOGAGAVAE traininig.")
...
def train_VAE(
    ...
    for epoch in range(epochs):
        ...
        for inputs, labels in train_loader:
            inputs = inputs.to(device)
            optimizer.zero_grad()
            x_re, _, mu, logvar = net(inputs) #  forward:  return x_re, mu, logvar
            loss, mse, kl = lossf.normal_loss(x_re, mu, logvar, inputs)
            # loss, ce, kl = lossf.ce_loss(x_re, mu, logvar, inputs, mse_w, kl_w)
            loss.backward()
            optimizer.step()
            train_run_loss += loss.item()
...
# __________________________________________________________________________________ #
        ...
        with torch.no_grad():
            for inputs, labels in val_loader:
                inputs = inputs.to(device)
                x_re, _, mu, logvar = net(inputs)
                loss, mse, kl = lossf.normal_loss(x_re, mu, logvar, inputs, mse_w, kl_w)
                # loss, mse, kl = lossf.ce_loss(x_re, mu, logvar, inputs, mse_w, kl_w)
                val_run_loss += loss.item()
        ...
        if epoch % 10 == 0:
            savename = f"prams/skull_VAE_epoch{epoch}.pth"
        #   torch.save(model.state_dict(), file_path)
            torch.save(net.to('cpu').state_dict(), path + savename)
    #       torch.save(model.state_dict(), log_path + f"softintroVAE_weight_epoch{str(epoch)}.pth")
            net = net.to(device)
        ...
        now_time = time.time()
        print(f"Epoch[{epoch+1}/{epochs}] Train[ce:{train_run_mse:.1f} kl:{train_run_kl:.1f} loss:{train_run_loss:.1f}]  "
              f"Val[ce:{val_run_mse:.1f} kl:{val_run_kl:.2f} loss:{val_run_loss:.1f}]  "
              f"1epoch:{(now_time - loop_start_time):.0f}秒  total:{(now_time - start_time)/60:.0f}分")
    ...
    if epochs != 0:
        net = net.to('cpu')
        torch.save(net.state_dict(), path + "last_resnetvae_weight.pth")
