...
import MRI_VAE.models.fcmodel as models
...
def save_image(image, output, epoch, path, b_size, train, fakeflag):
    ...
    if ((fakeflag is False) and (train is False)): # and 演算子注意
...
def train_soft_intro_vae(
    ...
    start_epoch = 0
    print(f"Hyper prams: beta_rec={beta_rec},  beta_neg={beta_neg},  beta_kl={beta_kl}, || scale = {scale:.6f}")
    # print("model = SoftIntroVAE(12, 24, 36, 48, 150)")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch, labels in train_loader:
            ...
            lossE_real_kl = calc_kl(real_logvar, real_mu, reduce="mean")# kl-divergence == lossE_real_kl (= scaler)
...
############# KLD loss for the fake data  ############ ここからコード確認する。###############################################
            ...
            # exp_elbo_fake=(-2*(scale*0.25 )*(beta_rec*loss_fake_rec+ beta_neg*fake_kl_e)).exp().mean()
            # exp_elbo_rec =(-2*(scale*0.25 )*(beta_rec*loss_rec_rec + beta_neg*rec_kl_e )).exp().mean()
...
#             0.25 * (exp_elbo_fake + exp_elbo_rec)=={0.25*(exp_elbo_fake+exp_elbo_rec):.4f}")
        ...
        train_run_mse /= (len(train_loader)*80*96*80) # mse
        ...
        with torch.no_grad():
            ...
            val_run_mse /= (len(val_loader)*80*96*80) # mse
...
#       torch.save(model.state_dict(), log_path + f"softintroVAE_weight_epoch{str(epoch)}.pth")
        ...
        print(f"Epoch[{epoch+1}/{epochs}] Train[RMSE:{train_run_mse:.5f} kl:{train_run_kl:.1f} loss:{train_run_loss:.1f}]  "
...
# __________________________________________________________________________________ #
        ...
        val_run_loss /= len(val_loader)
        val_run_mse /= (len(val_loader)*80*96*80)
        val_run_kl /= len(val_loader)
        val_losses.append(val_run_loss)
        val_mse_losses.append(val_run_mse)
        val_kl_losses.append(val_run_kl)
